...
struct common_params {
    ...
    // VRAM-aware eviction: layered on top of models_max (count-based) below. Before loading a
    // model, proactively evict LRU resident model(s) if the incoming model's estimated VRAM
    // footprint would not fit in currently-free GPU memory -- not just when the model *count*
    // hits models_max. Best-effort: requires `nvidia-smi` on PATH to query free VRAM (the router
    // process deliberately never touches the GPU itself, see server.cpp); silently falls back to
    // the models_max-only behavior if nvidia-smi is unavailable or a model's footprint can't be
    // estimated from its on-disk GGUF size (e.g. remote HF-repo presets not yet cached locally).
    bool models_vram_aware        = true; // whether to attempt VRAM-aware eviction at all
    int  models_vram_margin_mb    = 512;  // MiB of free VRAM to keep as a safety margin beyond a model's estimated footprint
    int  models_vram_overhead_pct = 20;   // percent overhead added to a model's on-disk GGUF size to approximate KV-cache/compute-buffer usage
