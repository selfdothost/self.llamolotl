...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--models-vram-aware"},
        {"--no-models-vram-aware"},
        string_format("for router server, whether to proactively evict LRU model(s) before a load whose "
                       "estimated VRAM footprint would not fit in currently-free GPU memory, in addition to "
                       "(not instead of) the models_max count limit; requires `nvidia-smi` on PATH and falls "
                       "back to the count-based limit alone if it's unavailable or a model's footprint can't "
                       "be estimated (default: %s)", params.models_vram_aware ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.models_vram_aware = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}).set_env("LLAMA_ARG_MODELS_VRAM_AWARE"));
    add_opt(common_arg(
        {"--models-vram-margin-mb"}, "N",
        string_format("for router server, MiB of free GPU VRAM to keep as a safety margin beyond a model's "
                       "estimated footprint when deciding whether VRAM-aware eviction is needed (default: %d)",
                       params.models_vram_margin_mb),
        [](common_params & params, int value) {
            params.models_vram_margin_mb = value;
        }
    ...
    ).set_examples({LLAMA_EXAMPLE_SERVER}).set_env("LLAMA_ARG_MODELS_VRAM_OVERHEAD_PCT"));
    add_opt(common_arg(
        {"--jinja"},
        {"--no-jinja"},
