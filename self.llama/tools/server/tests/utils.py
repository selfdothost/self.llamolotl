...
class ServerProcess:
    ...
    models_vram_aware: bool | None = None
    models_vram_margin_mb: int | None = None
    models_vram_overhead_pct: int | None = None
    # test-only: see LLAMA_TEST_FAKE_FREE_VRAM_MB(_FILE) in server-models.cpp's
    # query_free_vram_bytes() -- lets these tests exercise VRAM-aware eviction
    ...
    def start(self, timeout_seconds: int = DEFAULT_HTTP_TIMEOUT) -> None:
        ...
        if self.models_vram_aware is not None:
            server_args.append("--models-vram-aware" if self.models_vram_aware else "--no-models-vram-aware")
        if self.models_vram_margin_mb is not None:
            server_args.extend(["--models-vram-margin-mb", self.models_vram_margin_mb])
        if self.models_vram_overhead_pct is not None:
            server_args.extend(["--models-vram-overhead-pct", self.models_vram_overhead_pct])
        if self.fake_free_vram_mb is not None:
