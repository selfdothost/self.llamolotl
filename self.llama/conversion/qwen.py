...
class _LinearAttentionVReorderBase(Qwen3NextModel):
    ...
    def _reorder_v_heads(tensor: Tensor, dim: int, num_k_heads: int, num_v_per_k: int, head_dim: int) -> Tensor:
        ...
        # Detect LoRA tensors via duck-typing (has get_lora_A_B method). Needed
        # because LoraTorchTensor's own reshape()/permute() don't support the
        # multi-dim split-and-swap this function does (self.llamolotl, ported
        # forward across the 2026-07-16 re-vendor from upstream commit 699bb0e
        # via self.llamolotl!24's cavekit-llama-revendor.md R2/R3).
        if hasattr(tensor, 'get_lora_A_B'):
            # For LoRA tensors (B @ A decomposition), apply the reorder to the
            # correct component based on which dimension is being reordered.
            lora_A, lora_B = tensor.get_lora_A_B()
            ndim = len(tensor.shape)
            actual_dim = dim if dim >= 0 else dim + ndim
            if actual_dim == ndim - 1:
                # Reordering the last dim (row/input) -> apply to A's last dim
                reordered_A = _LinearAttentionVReorderBase._reorder_v_heads(
                    lora_A, len(lora_A.shape) - 1, num_k_heads, num_v_per_k, head_dim)
                return type(tensor)(reordered_A, lora_B)
            else:
                # Reordering a batch/col dim -> apply to B's corresponding dim
                reordered_B = _LinearAttentionVReorderBase._reorder_v_heads(
                    lora_B, actual_dim, num_k_heads, num_v_per_k, head_dim)
                return type(tensor)(lora_A, reordered_B)
